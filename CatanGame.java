...
public class CatanGame extends HttpServlet {
	...
	public synchronized void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		boolean moveKnight = false, stealResource = false, roadBuilding1 = false, roadBuilding2 = false, yearOfPlenty = false, monopoly = false, trading = false, discardResources = false, endTurn = false;
			...
			for (Player player : players) {
				if (player.getName().equals(playerName)) {
					if ((longestRoadHolder.equals("null") && player.getRoads().size() >= 5) || (!longestRoadHolder.equals("null") && player.getRoads().size() > longestRoad)) {
						if (!longestRoadHolder.equals("null")) {
							for (Player temp : players) {
								if (temp.getName().equals(longestRoadHolder)) {
									temp.setPoints(player.getPoints()-2);
								}
							}
						}
			...
			for (Player player : players) {
				if (player.getName().equals(playerName)) {
					if ((largestArmyHolder.equals("null") && player.getArmySize() >= 3) || (!largestArmyHolder.equals("null") && player.getArmySize() > largestArmy)) {
						if (!largestArmyHolder.equals("null")) {
							for (Player temp : players) {
								if (temp.getName().equals(largestArmyHolder)) {
									temp.setPoints(player.getPoints()-2);
								}
							}
						}
		...
		// moving knight
			...
			if (request.getParameter("playerToStealFrom").equals("selectPlayer")) {
				...
				for (Player player : players) {
					if (player.getName().equals(request.getParameter("playerToStealFrom")) && player.getResources().size() != 0) {
		...
		// from player_pages
		// initial road and settlement
		else if (request.getParameter("initialSettlementAndRoad") != null) {
			String roadLoc1 = request.getParameter("location1"); String roadLoc2 = request.getParameter("location2"); String settlementLoc = request.getParameter("location");
			if (roadLoc1.equals("selectLocation") || roadLoc2.equals("selectLocation") || (!roadLoc1.equals(settlementLoc) && !roadLoc2.equals(settlementLoc)) || settlementLoc.equals("selectLocation")) {
				addUpdate(response, "<strong>WARNING!</strong> One or more errors has occurred. Please resubmit.", false);
			} else {
				makeRoad(response, playerName, roadLoc1, roadLoc2, false);
				assignSettlement(playerName, settlementLoc, response);
				endTurn = true;
			}
			action = true;
		}
		...
		// from player_pages
		// ending turn
		if (request.getParameter("endTurn") != null || endTurn) {
	...
	public void writePlayerPages(HttpServletResponse response, String currentPlayer, boolean moveKnight, boolean stealResource, boolean roadBuilding1, boolean roadBuilding2, boolean yearOfPlenty, boolean monopoly, boolean trading, String player2Name, String tradingString, boolean discardResources, boolean turnPart2) throws IOException {
		...
		for (Player player : players) {
			try {
				...
				// DISCARDING RESOURCES
				String discardResourcesList = "<table style=\"width:400px\"><tr><form action=\"CatanGame\" method=\"POST\"><th style=\"padding:10px;background-color:red;color:black\">Select Half of Your Resources to Discard:<br>";
				counter = 1;
				for (Resource resource : player.getResources()) {
				...
				String playersToStealFromList = "<table><tr><form action=\"CatanGame\" method=\"POST\"><th style=\"padding:10px;background-color:red;color:black;width:350px\">Select the Player to Steal From: <select name=\"playerToStealFrom\"><option value=\"selectPlayer\">Select Player</option>";
				for (Player temp : players) {
					boolean added = false;
				...
				output = output.replaceAll("CatanGame", "http://192.168.1.15:8080/examples/servlets/servlet/CatanGame");
	...
	public String getPlayerTurnPart2(HttpServletResponse response, String currentPlayer, String output) throws IOException {
		ArrayList<String> unavailableSettlementsLocations = new ArrayList<>();
		for (Player player : players) {
			for (Settlement settlement : player.getSettlements()) {
				for (String location2 : getAccessibleLocations2(settlement.getLocationName())) {
					unavailableSettlementsLocations.add(location2);
				}
			}
		}
		...
		for (Player player : players) {
			if (player.getName().equals(currentPlayer)) {
				...
				String potentialSettlementLocationsList = "";
				for (String location1 : player.getAccessibleLocations()) {
					accessibleLocations1List += "<option value=\"" + location1 + "\">" + location1 + "</option>\n";
					if (player.getSettlementsList().indexOf(location1) == -1 && !unavailableSettlementsLocations.contains(location1)) {
						potentialSettlementLocationsList += "<option value=\"" + location1 + "\">" + location1 + "</option>\n";
					}
				...
				output = output.replace("*POTENTIAL SETTLEMENT LOCATIONS", potentialSettlementLocationsList);
	...
	public void showLogin(HttpServletResponse response) throws IOException {
		...
		try {
			FileWriter myWriter = new FileWriter("/Applications/MAMP/tomcat/webapps/examples/servlets/general_login.html");
			myWriter.write("<html><form action=\"http://192.168.1.15:8080/examples/servlets/servlet/CatanGame\" method=\"POST\">Who's Playing Today? <select name=\"whosPlaying\"><option value=\"selectPlayer\">Select Player</option>" + list + "</select> <input type=\"submit\" value=\"Submit\"/><br></form><p style=\"color:red\"><strong>NOTE: A random ordering will be determined once game begins.</strong></p></html>");
			myWriter.close();
		} catch (IOException e) {
			e.printStackTrace();
		}

		out.println("<html><h1>Congrats! Your Game has Started.</h1><h2>please have all players go to the following website:<br><a href=\"http://192.168.1.15:8080/examples/servlets/general_login.html\">http://192.168.1.15:8080/examples/servlets/general_login.html</a></h2></html>");
	}
		...
		while (input.hasNextLine()) {
			...
			if (line.contains("*STATISTICS")) {
				...
				for (Player player : players) {
					...
					line += "<p style=\"padding-top:5px;padding-bottom:5px;padding-left:10px;padding-right:5px;color:" + textColor + ";background:" + backgroundColor + "\">" + player.toFormattedString() + "<br></p>";
				}
			}
	...
	public String getUpdates() {
		Scanner input = readFile(readMeLocation);
		String updates1 = "", updates2 = "";
		while (input.hasNextLine()) {
			String line = input.nextLine();
			if (line.length() > 2 && line.substring(0,2).equals("8:")) {
				String[] parts = line.split(":");
				if (parts.length <= 5) {
					updates1 += "<mark>" + parts[parts.length-1] + "</mark><br>";
					for (int i = parts.length-2; i > 0; i--) {
						updates1 += parts[i] + "<br>";
					}
				} else {
					updates1 += "<mark>" + parts[parts.length-1] + "</mark><br>";
					for (int i = parts.length-2; i > parts.length-5; i--) {
						updates1 += parts[i] + "<br>";
					}
					if (parts.length <= 9) {
						for (int i = parts.length-5; i > 0; i--) {
							updates2 += parts[i] + "<br>";
						}
					} else {
						for (int i = parts.length-5; i > parts.length-9; i--) {
							updates2 += parts[i] + "<br>";
						}
					}
				}
		...
		return "<div class=\"topnav\"><table style=\"width:100%;height:70px\"><tr><th style=\"padding-left:10px;padding-top:5px;padding-bottom:5px;color:black;font-size:20px;width:20%\">Recent Updates:</th><th style=\"padding-left:10px;padding-right:5px;color:black;font-size:15px;width:40%;border:1px solid black\">" + updates1 + "</th><th style=\"padding-left:10px;padding-right:5px;color:black;font-size:15px;width:40%;border:1px solid black\">" + updates2 + "</th></tr></table></div>";
	}
